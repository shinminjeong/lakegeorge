...
ggplot()+
  geom_line(data=orderdiv[orderdiv$order=="Poales",], aes(x=as.POSIXct(as.character(year), format = "%Y"), y=Freq/yeardiv$Freq, color=order, group=order))+
  # geom_line(data=lakegeorge, aes(x=as.POSIXct(as.character(year), format = "%Y"), y=water, colour=water)) +  
  ...
  # scale_x_discrete(limits=c(1986:2018))+
